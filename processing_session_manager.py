...
class ProcessingSessionManager:
    ...
    def _create_session_info_file(self, session_folder: Path, session_id: str, 
        ...
        # NEW: Build calibration frames table
        calibration_table = self._build_calibration_table(session_id, files)
...
"""
    ...
    def _build_calibration_table(self, session_id: str, files: Optional[List[FitsFile]] = None) -> str:
        """Build calibration frames table grouped by imaging session."""
        
        # If files not provided, query from database
        if files is None:
            session = self.db_service.db_manager.get_session()
            try:
                # Get all calibration frames for this session
                cal_files = session.query(FitsFile).join(ProcessingSessionFile).filter(
                    ProcessingSessionFile.processing_session_id == session_id,
                    FitsFile.frame_type.in_(['DARK', 'FLAT', 'BIAS'])
                ).all()
            finally:
                session.close()
        else:
            # Filter to only calibration frames from provided files
            cal_files = [f for f in files if f.frame_type in ['DARK', 'FLAT', 'BIAS']]
        
        if not cal_files:
            return "_No calibration frames in this session_"
        
        # Group by imaging session, frame type, filter (for flats), exposure (for darks), and binning
        from collections import defaultdict
        ...
        for f in cal_files:
            ...
            if frame_type == "DARK":
                ...
                exposure = "-"  # Ignore exposure for FLATS and BIAS
            binning = f.binning_x if f.binning_x else 1
            
            # Create key tuple
            key = (imaging_session, date, frame_type, filter_name, exposure, binning)
            calibration_data[key] += 1
        
        # Sort by imaging session, frame type, then filter
        sorted_data = sorted(calibration_data.items(), key=lambda x: (x[0][0], x[0][2], x[0][3]))
        
        # Build markdown table
        table = "| Imaging Session        | Date       | Type  | Filter   | Number | Duration (s) | Binning |\n"
        table += "|------------------------|------------|-------|----------|--------|--------------|---------|"
        
        for (imaging_session, date, frame_type, filter_name, exposure, binning), count in sorted_data:
            # Wrap imaging session in [[]] for Obsidian linking
            obsidian_link = f"[[{imaging_session}]]"
            # For flats show filter, for darks show exposure, for bias show dash
            if frame_type == 'FLAT':
                detail = filter_name
            ...
            table += f"\n| {obsidian_link:<22} | {date:<10} | {frame_type:<5} | {detail:<8} | {count:>6} | {exposure:>12} | {binning:>7} |"
    ...
    def _build_acquisition_table(self, session_id: str, files: Optional[List[FitsFile]] = None) -> str:
        ...
        # Group by imaging session, date, filter, exposure, and binning
        from collections import defaultdict
        ...
        for f in light_files:
            ...
            # Create key tuple - now includes imaging session
            key = (imaging_session, date, filter_name, exposure, binning)
            acquisition_data[key] += 1
        
        # Sort by imaging session, then date, then filter
        sorted_data = sorted(acquisition_data.items(), key=lambda x: (x[0][0], x[0][1], x[0][2]))
        
        # Build markdown table with aligned columns - imaging session first
        table = "| Imaging Session        | Date       | Filter        | Number | Duration (s) | Binning |\n"
        table += "|------------------------|------------|---------------|--------|--------------|---------|"
        
        for (imaging_session, date, filter_name, exposure, binning), count in sorted_data:
            # Wrap imaging session in [[]] for Obsidian linking
            obsidian_link = f"[[{imaging_session}]]"
            table += f"\n| {obsidian_link:<22} | {date:<10} | {filter_name:<13} | {count:>6} | {exposure:>12} | {binning:>7} |"
    ...
    def delete_processing_session(self, session_id: str, remove_files: bool = True) -> bool:
        ...
        try:
            ...
            # NEW: Always delete the session notes file
            try:
                # Extract year from session_id (format: YYYYMMDD_...)
                try:
                    year = int(session_id[:4])
                ...
                if notes_file.exists():
                    notes_file.unlink()
                    logger.info(f"Removed processing session notes: {notes_file}")
            except Exception as notes_error:
                # Don't fail the whole deletion if notes deletion fails
                logger.warning(f"Failed to delete notes file for {session_id}: {notes_error}")
